...
**"Construct N quests."** (optional first line) — produce exactly N objects in the array instead of 3. All other rules hold at batch scale: still at most 1 food-flavored quest in the whole batch, never repeat a §6 template within the batch, and when N is 6 spread the batch across at least 4 different categories (unless a single category was requested). The §5 per-batch caps (C1–C9) also apply to the WHOLE set of N: no two candidates built around the same central prop/object/material (C8), no two using the same capped mechanic family (C2–C7), no two sharing a core verb (C9) — the server hard-drops such duplicates, so a second variation of an idea is a wasted slot. The server ranks and ships the best 3, so N distinct, rule-clean candidates beat N variations of one idea.
